...
class TestTaskManager(unittest.TestCase):
	...
	def test_list_tasks(self):
		tasks = [
		    Task("Task 1 all", "Description 1 all"),
		    Task("Task 2 all", "Description 2 all"),
		    Task("Task 3 all", "Description 3 all")
		]
	...
	def test_list_tasks_exclude_completed(self):
		...
		self.storage.get_all_tasks.return_value = tasks
		result = self.manager.list_tasks()
		pending_tasks_result = self.manager.list_tasks(False)
		self.assertEqual(len(result), 3)
		# self.assertNotIn(tasks[1], result)
		self.assertNotIn(tasks[1], pending_tasks_result)
